...
def upgrade():
    ...
    op.drop_constraint(
        "fk_auto_labels_raw_chapter_revision_id_raw_chapter_revisions", "auto_labels", type_="foreignkey"
    )

    # label_datas
    op.drop_constraint("one_label_group_per_chapter", "label_datas", type_="unique")
    op.drop_constraint(
        "fk_label_datas_raw_chapter_revision_id_raw_chapter_revisions", "label_datas", type_="foreignkey"
    )

    # raw_chapter_revisions
    op.drop_constraint(
        "fk_raw_chapter_revisions_raw_chapter_id_raw_chapters", "raw_chapter_revisions", type_="foreignkey"
    )
    ...
    # chapters
    op.execute(
        "ALTER TABLE chapters RENAME CONSTRAINT raw_chapters_raw_chapter_id_not_null TO chapters_chapter_id_not_null"
    )
    op.execute(
        "ALTER TABLE chapters RENAME CONSTRAINT raw_chapters_raw_chapter_num_not_null TO chapters_chapter_num_not_null"
    )
    op.execute("ALTER TABLE chapters RENAME CONSTRAINT raw_chapters_novel_id_not_null TO chapters_novel_id_not_null")
    op.execute(
        "ALTER TABLE chapters RENAME CONSTRAINT raw_chapters_created_at_not_null TO chapters_created_at_not_null"
    )
    op.execute(
        "ALTER TABLE chapters RENAME CONSTRAINT raw_chapters_updated_at_not_null TO chapters_updated_at_not_null"
    )

    # revisions
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_revision_id_not_null TO revisions_revision_id_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_revision_text_not_null TO revisions_revision_text_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_revision_title_not_null TO revisions_revision_title_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_revision_is_primary_not_null TO revisions_revision_is_primary_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_revision_is_public_not_null TO revisions_revision_is_public_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_revision_is_final_not_null TO revisions_revision_is_final_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_raw_chapter_id_not_null TO revisions_chapter_id_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_created_at_not_null TO revisions_created_at_not_null"
    )
    op.execute(
        "ALTER TABLE revisions RENAME CONSTRAINT raw_chapter_revisions_updated_at_not_null TO revisions_updated_at_not_null"
    )

    # label_datas
    op.execute(
        "ALTER TABLE label_datas RENAME CONSTRAINT label_datas_raw_chapter_revision_id_not_null TO label_datas_revision_id_not_null"
    )

    # auto_labels
    op.execute(
        "ALTER TABLE auto_labels RENAME CONSTRAINT auto_labels_raw_chapter_revision_id_not_null TO auto_labels_revision_id_not_null"
    )
    ...
    op.create_check_constraint(
        "primary_must_be_public_check", "revisions", "revision_is_public OR NOT revision_is_primary"
    )
    op.create_index(
        ...
        ["chapter_id"],
        unique=True,
        postgresql_where=sa.text("revision_is_primary IS true"),
    )
    ...
    op.create_foreign_key(
        "fk_label_datas_revision_id_revisions", "label_datas", "revisions", ["revision_id"], ["revision_id"]
    )

    # --- recreate constraints: auto_labels ---

    op.create_unique_constraint(
        "uq_model_name_params", "auto_labels", ["revision_id", "auto_label_model_name", "auto_label_model_params"]
    )
    op.create_foreign_key(
        "fk_auto_labels_revision_id_revisions", "auto_labels", "revisions", ["revision_id"], ["revision_id"]
    )
...
def downgrade():
    ...
    # raw_chapters
    op.execute(
        "ALTER TABLE raw_chapters RENAME CONSTRAINT chapters_chapter_id_not_null TO raw_chapters_raw_chapter_id_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapters RENAME CONSTRAINT chapters_chapter_num_not_null TO raw_chapters_raw_chapter_num_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapters RENAME CONSTRAINT chapters_novel_id_not_null TO raw_chapters_novel_id_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapters RENAME CONSTRAINT chapters_created_at_not_null TO raw_chapters_created_at_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapters RENAME CONSTRAINT chapters_updated_at_not_null TO raw_chapters_updated_at_not_null"
    )

    # raw_chapter_revisions
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_revision_id_not_null TO raw_chapter_revisions_raw_chapter_revision_id_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_revision_text_not_null TO raw_chapter_revisions_raw_chapter_revision_text_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_revision_title_not_null TO raw_chapter_revisions_raw_chapter_revision_title_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_revision_is_primary_not_null TO raw_chapter_revisions_raw_chapter_revision_is_primary_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_revision_is_public_not_null TO raw_chapter_revisions_raw_chapter_revision_is_public_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_revision_is_final_not_null TO raw_chapter_revisions_raw_chapter_revision_is_final_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_chapter_id_not_null TO raw_chapter_revisions_raw_chapter_id_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_created_at_not_null TO raw_chapter_revisions_created_at_not_null"
    )
    op.execute(
        "ALTER TABLE raw_chapter_revisions RENAME CONSTRAINT revisions_updated_at_not_null TO raw_chapter_revisions_updated_at_not_null"
    )

    # label_datas
    op.execute(
        "ALTER TABLE label_datas RENAME CONSTRAINT label_datas_revision_id_not_null TO label_datas_raw_chapter_revision_id_not_null"
    )

    # auto_labels
    op.execute(
        "ALTER TABLE auto_labels RENAME CONSTRAINT auto_labels_revision_id_not_null TO auto_labels_raw_chapter_revision_id_not_null"
    )
    ...
    op.create_foreign_key(
        "fk_raw_chapter_revisions_raw_chapter_id_raw_chapters",
        "raw_chapter_revisions",
        "raw_chapters",
        ["raw_chapter_id"],
        ["raw_chapter_id"],
    ...
    op.create_check_constraint(
        "primary_must_be_public_check",
        "raw_chapter_revisions",
        "raw_chapter_revision_is_public OR NOT raw_chapter_revision_is_primary",
    )
    op.create_index(
        "ix_one_primary_revision_per_chapter",
        "raw_chapter_revisions",
        ["raw_chapter_id"],
        unique=True,
        postgresql_where=sa.text("raw_chapter_revision_is_primary IS true"),
    )

    # --- recreate constraints: label_datas ---

    op.create_unique_constraint(
        "one_label_group_per_chapter", "label_datas", ["label_group_id", "raw_chapter_revision_id"]
    )
    op.create_foreign_key(
        "fk_label_datas_raw_chapter_revision_id_raw_chapter_revisions",
        "label_datas",
        "raw_chapter_revisions",
    ...
    op.create_unique_constraint(
        "uq_model_name_params",
        "auto_labels",
        ["raw_chapter_revision_id", "auto_label_model_name", "auto_label_model_params"],
    )
    op.create_foreign_key(
        "fk_auto_labels_raw_chapter_revision_id_raw_chapter_revisions",
        "auto_labels",
        "raw_chapter_revisions",
        ["raw_chapter_revision_id"],
        ["raw_chapter_revision_id"],
