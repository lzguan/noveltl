...
def _lock_workflows(db: Session, workflow_ids: tuple[UUID, ...]) -> dict[UUID, _WorkflowState]:
    ...
    return {
        workflow_id: _WorkflowState(status=status, job_id=job_id)
        for workflow_id, status, job_id in workflows
    }
...
def queue_fjob(
    ...
    *,
    ...
    if any(
        locked_workflows[workflow_id].status not in member_workflow_states
        for workflow_id in member_workflow_ids
    ):
        return False
    if any(
        locked_groupings[grouping_id].status not in member_grouping_states
    ...
    if any(
        locked_workflows[workflow_id].status not in at_rest_workflow_states
        for workflow_id in neighbour_workflow_ids
    ):
        return False
    if any(
        locked_groupings[grouping_id].status not in at_rest_grouping_states
...
def _transition_fjob(
    ...
    if any(
        state.status != workflow_from or state.job_id != job_id
        for state in workflows.values()
    ):
        return False
    if any(
        state.status != grouping_from or state.job_id != job_id
