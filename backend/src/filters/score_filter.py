...
class ScoreFilter(
    ...
    def flag_instances(self, db: Session, current_user: User, options: ScoreFlagInstancesOptions) -> list[SingleLabel]:
        q = (
            select(label_models.Label, novel_models.ChapterContent.chapter_content_id)
            ...
            .join(novel_models.Chapter, novel_models.ChapterContent.chapter_id == novel_models.Chapter.chapter_id)
        ...
        return [
            SingleLabel(label=label_schemas.Label.model_validate(label), chapter_content_id=id)
    ...
    def get_contexts(
        ...
        q = (
            select(novel_models.ChapterContent)
            .where(novel_models.ChapterContent.chapter_content_id.in_(chapter_content_ids))
            .join(novel_models.Chapter, novel_models.ChapterContent.chapter_id == novel_models.Chapter.chapter_id)
            .where(
                novel_models.ChapterContent.chapter_content_version
                == select(func.max(novel_models.ChapterContent.chapter_content_version))
    ...
    def apply_filter(
        ...
        stmt = (
            ...
            .where(
                tuple_(
                    select(sub_q.c.chapter_content_id)
