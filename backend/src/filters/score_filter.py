...
class ScoreFilter(
    ...
    def flag_instances(self, db: Session, current_user: User, options: ScoreFlagInstancesOptions) -> list[SingleLabel]:
        q = (
            select(label_models.Label, novel_models.RevisionText.revision_text_id)
            .where(label_models.Label.label_score < options.min_score)
            ...
            .join(novel_models.Revision, novel_models.RevisionText.revision_id == novel_models.Revision.revision_id)
            .join(novel_models.Chapter, novel_models.Revision.chapter_id == novel_models.Chapter.chapter_id)
            .where(label_models.LabelData.label_group_id == options.label_group_id)
            .where(
        ...
        return [
            SingleLabel(label=label_schemas.Label.model_validate(label), revision_text_id=id)
            for label, id in result_rows
        ]

    def get_contexts(
        self, db: Session, current_user: User, instances: list[SingleLabel], options: ScoreGetContextOptions
    ) -> list[SentenceContext | None]:
        revision_text_ids = list({instance.revision_text_id for instance in instances})
        q = (
            select(novel_models.RevisionText)
            .where(novel_models.RevisionText.revision_text_id.in_(revision_text_ids))
            .join(novel_models.Revision, novel_models.RevisionText.revision_id == novel_models.Revision.revision_id)
            .where(
                novel_models.RevisionText.revision_text_version
                == select(func.max(novel_models.RevisionText.revision_text_version))
    ...
    def _check_instances_not_stale(self, db: Session, current_user: User, revision_text_ids: set[uuid.UUID]) -> None:
        ...
        if not revision_text_ids:
            return
        q = (
            select(novel_models.RevisionText.revision_text_id)
            .join(novel_models.Revision, novel_models.RevisionText.revision_id == novel_models.Revision.revision_id)
            .where(novel_models.RevisionText.revision_text_id.in_(revision_text_ids))
            .where(
                novel_models.RevisionText.revision_text_version
    ...
    def apply_filter(
        self, db: Session, current_user: User, instances: list[SingleLabel], options: ScoreApplyFilterOptions
    ) -> None:
        ...
        sub_q = select(label_models.LabelData).where(label_models.LabelData.label_group_id == label_group_id).subquery()

        stmt = (
            delete(label_models.Label)
            .where(label_models.Label.label_data_id.in_(select(sub_q.c.label_data_id)))
            .where(
                tuple_(
                    select(sub_q.c.revision_text_id)
                    .where(sub_q.c.label_data_id == label_models.Label.label_data_id)
                    .correlate(label_models.Label)
                    .scalar_subquery(),
