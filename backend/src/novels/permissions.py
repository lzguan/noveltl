...
def chapter_mod_access_select[T: Select[tuple[Any, ...]]](q: T, current_user: User | None) -> T:
    ...
    if current_user is None:
        ...
        sub_q = select(1).select_from(Chapter).join(Novel, Chapter.novel_id == Novel.novel_id).correlate(Chapter)
...
def revision_mod_access_select[T: Select[tuple[Any, ...]]](q: T, current_user: User | None) -> T:
    ...
    if current_user is None:
        sub_q = (
            ...
            .where(Revision.revision_is_public.is_(True))
            .join(Chapter, Revision.chapter_id == Chapter.chapter_id)
            .join(Novel, Chapter.novel_id == Novel.novel_id)
            .correlate(Revision)
        )
        ...
        sub_q = (
            ...
            .where(
                or_(
                    and_(Novel.novel_visibility >= Visibility.UNLISTED, Revision.revision_is_public.is_(True)),
                    exists(
                        select(1)
                        .select_from(Contributor)
                        .where(Contributor.novel_id == Novel.novel_id)
...
def revision_text_mod_access_select[T: Select[tuple[Any, ...]]](q: T, current_user: User | None) -> T:
    subq = (
        select(1).select_from(Revision).where(Revision.revision_id == RevisionText.revision_id).correlate(RevisionText)
    )
    ...
    if current_user.user_type != UserType.ADMIN:
        return stmt.where(
            exists(
                select(1)
                .select_from(Contributor)
                .where(Contributor.novel_id == Chapter.novel_id)
                .where(Contributor.user_id == current_user.user_id)
                .where(Contributor.contributor_role.in_([Role.OWNER, Role.EDITOR]))
            )
        )
    ...
    if current_user.user_type != UserType.ADMIN:
        return stmt.where(
            exists(
                ...
                .where(
                    Contributor.novel_id
                    == select(Chapter.novel_id).where(Chapter.chapter_id == Revision.chapter_id).scalar_subquery()
                )
                .where(Contributor.user_id == current_user.user_id)
                .where(Contributor.contributor_role.in_([Role.OWNER, Role.EDITOR]))
            )
    ...
    if current_user.user_type != UserType.ADMIN:
        return stmt.where(
            exists(
                ...
                .where(Contributor.user_id == current_user.user_id)
                .where(Contributor.contributor_role == Role.OWNER)
            )
        )
    return stmt


def revision_text_mod_access_insert[T: Select[tuple[Any, ...]]](
    stmt: T, current_user: User, revision_id: uuid.UUID
) -> T:
    if current_user.user_type != UserType.ADMIN:
        return stmt.where(
            exists(
                ...
                .where(
                    ...
                    .where(
                        Chapter.chapter_id
                        == select(Revision.chapter_id).where(Revision.revision_id == revision_id).scalar_subquery()
                    )
                    .scalar_subquery()
                )
                .where(Contributor.user_id == current_user.user_id)
