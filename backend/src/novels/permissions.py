...
def novel_mod_access_select[T : Select[tuple[Any, ...]]](q : T, current_user : User | None, aliased_type : type[Novel] = Novel) -> T:
    """
        ...
        return q.where(or_(
            ...
            exists(
                select(
                    ...
                    NovelContributor.novel_id == aliased_type.novel_id
                ).where(
        ...
        return stmt.where(
            exists(
                select(
                    ...
                    NovelContributor.novel_id == Novel.novel_id
                ).where(
                    NovelContributor.user_id == current_user.user_id
                ).where(
...
def source_work_mod_access_select[T : Select[tuple[Any, ...]]](q : T, current_user : User | None, aliased_type : type[SourceWork] = SourceWork) -> T:
    """
    Takes a select statement for source works and returns a select statement that restricts permissions on source works.
    """
    subq = select(1).select_from(Novel).where(
        Novel.source_work_id == aliased_type.source_work_id).correlate(aliased_type)
    ...
    if current_user.user_type != UserType.ADMIN:
        return stmt.where(
            exists(
                select(
                    1
                ).select_from(Novel).where(
                    Novel.source_work_id == SourceWork.source_work_id
                ).where(
                    exists(
                        select(
                            1
                            ...
                            NovelContributor.contributor_role.in_([Role.OWNER])
                        )
                    )
                )
            )
...
def chapter_mod_access_select[T : Select[tuple[Any, ...]]](q : T, current_user : User | None, aliased_type : type[Chapter] = Chapter) -> T:
    """
    ...
    if current_user is None:
        ...
        sub_q = select(
            ...
            aliased_type.novel_id == Novel.novel_id
        ...
        return stmt.where(
            exists(
                select(
                    ...
                    NovelContributor.novel_id == Chapter.novel_id
                ).where(
                    NovelContributor.user_id == current_user.user_id
                ).where(
                    NovelContributor.contributor_role.in_([Role.OWNER, Role.EDITOR])
        ...
        return stmt.where(
            exists(
                select(
                    ...
                    NovelContributor.novel_id == novel_id
                ).where(
                    NovelContributor.user_id == current_user.user_id
                ).where(
                    NovelContributor.contributor_role.in_([Role.OWNER, Role.EDITOR])
        ...
        return stmt.where(
            exists(
                select(
                    ...
                    NovelContributor.contributor_role == Role.OWNER
                )
            )
        )
    return stmt

def chapter_content_mod_access_select[T : Select[tuple[Any, ...]]](q : T, current_user : User | None, aliased_type : type[ChapterContent] = ChapterContent) -> T:
    subq = select(1).select_from(Chapter).where(
        Chapter.chapter_id == aliased_type.chapter_id
    ).correlate(aliased_type)
    subq = chapter_mod_access_select(subq, current_user)
    return q.where(exists(subq))

def chapter_content_mod_access_insert[T : Select[tuple[Any, ...]]](stmt : T, current_user : User, chapter_id : uuid.UUID) -> T:
    if current_user.user_type != UserType.ADMIN:
        return stmt.where(exists(
            select(1).select_from(NovelContributor).where(
                NovelContributor.novel_id == select(Chapter.novel_id).where(
