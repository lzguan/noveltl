...
@pytest.fixture
def sample_novels(
    sample_languages: dict[str, Language], test_db: Session, sample_source_work: SourceWork
) -> list[Novel]:
    # Create some sample novels
    novel0 = Novel(
        novel_title="Sample Novel 1",
        language_code=sample_languages["en"].language_code,
        novel_type=NovelType.ORIGINAL,
        novel_visibility=Visibility.PUBLIC,
        source_work_id=sample_source_work.source_work_id,
    )
    novel1 = Novel(
        novel_title="Sample Novel 2",
        language_code=sample_languages["zh"].language_code,
        novel_type=NovelType.ORIGINAL,
        novel_visibility=Visibility.PUBLIC,
        source_work_id=sample_source_work.source_work_id,
    )
    novel2 = Novel(
        novel_title="Sample Novel 3",
        language_code=sample_languages["kr"].language_code,
        novel_description="A description.",
        novel_author="An Author",
        novel_type=NovelType.ORIGINAL,
        novel_visibility=Visibility.PUBLIC,
        source_work_id=sample_source_work.source_work_id,
    )
    novel3 = Novel(
        novel_title="Smample Novel 4",
        language_code=sample_languages["zh"].language_code,
        novel_description="Another description.",
        novel_author="Another Author",
        novel_type=NovelType.ORIGINAL,
        novel_visibility=Visibility.PUBLIC,
        source_work_id=sample_source_work.source_work_id,
    )
...
def sample_contributors(
    test_db: Session, sample_novels: list[Novel], sample_users: list[User]
) -> list[NovelContributor]:
    # Assign the regular user as the OWNER of Novel 1 and EDITOR of Novel 2
    c1 = NovelContributor(
        novel_id=sample_novels[1].novel_id, user_id=sample_users[1].user_id, contributor_role=Role.OWNER
    )
    c2 = NovelContributor(
        novel_id=sample_novels[2].novel_id, user_id=sample_users[1].user_id, contributor_role=Role.EDITOR
    )
...
@pytest.fixture
def sample_label_contributors(
    test_db: Session, sample_label_groups: list[LabelGroup], sample_users: list[User]
) -> list[LabelContributor]:
...
@pytest.fixture
def sample_label_datas(
    test_db: Session, sample_label_groups: list[LabelGroup], sample_chapter_contents: list[ChapterContent]
) -> list[LabelData]:
    # Link the Label Data to the first ChapterContent of Chapter 1
    cc1 = sample_chapter_contents[0]
    ld1 = LabelData(label_group_id=sample_label_groups[0].label_group_id, chapter_content_id=cc1.chapter_content_id)
    test_db.add(ld1)
    test_db.commit()
    test_db.refresh(ld1)
    return [ld1]
