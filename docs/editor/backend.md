...
This workflow is implemented as a pair of decorators in [backend/src/requests/decorators.py](../../backend/src/requests/decorators.py) — `ttl_cache` for synchronous endpoints and `attl_cache` for async endpoints — together with an `serialize_response_model` helper that serializes the success payload. Cached editor endpoints include `POST /label-groups`, `POST /label-groups/{labelGroupId}/label-datas`, `PATCH /label-datas/{labelDataId}`, and `PATCH /chapters/{chapterId}/content`.
