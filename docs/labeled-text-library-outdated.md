...
Deprecated design doc. The current implementation lives in [labeled-text-library.md](labeled-text-library.md).

This document is still useful for original motivation and possible future directions, but it does not accurately describe the current code under `frontend/src/components/labeled-text-lib/`.
