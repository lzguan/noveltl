...
import { labelGroupsWithRole, loginByApi, loginToken } from "../helpers/api";
...
test("shows label groups in the editor only for the contributing user", async ({ browser, request }) => {
  const seed = readSeed();
