...
export type ManagedSegment<S extends Style, L extends StyledLabel<S>> = Segment<
  S,
  ManagedLabel<S, L>
> & {
  id: SegmentID;
...
export function makeBasicSegmentManager<S extends Style, L extends StyledLabel<S>>(
  initialText: string,
  initialLabels: ManagedLabel<S, L>[],
  gap: number = 0,
): SegmentManager<S, L> {
  const segmenter = makeBasicSegmenter<S, ManagedLabel<S, L>>(gap);
  const segmentManager: SegmentManager<S, L> &
    SegmentManagerInternals<S, L> &
    SegmentManagerData<S, L> = {
    ...
    mergeSegments(startIdx: Index, endIdx: Index) {
      ...
      const segments = boundsSegments.map(
        (b) => this.segmentsById.get(b.id) as Segment<S, ManagedLabel<S, L>>,
      );
      ...
      //    l4:             44
        ...
        if (relStart - leftUp + (rightDown - relEnd) < gap) {
          ...
          if (leftUp === 0 && rightDown === segment.text.length) {
            ...
            newSegment.text =
              newSegment.text.slice(0, relStart - leftUp) + newSegment.text.slice(relEnd - leftUp);
        ...
        if (killStart - 1 >= 0 && killStart < this.bounds.length) {
          const rightDown = Math.min(
            ...
            ...this.getSegment(this.bounds[killStart].id).labels.map((l) => l.interval.start),
          );
          const leftUp = Math.max(
            0,
            ...this.getSegment(this.bounds[killStart - 1].id).labels.map((l) => l.interval.end),
          );
          if (
            rightDown + this.bounds[killStart].start - this.bounds[killStart - 1].start - leftUp <
            gap
          ) {
