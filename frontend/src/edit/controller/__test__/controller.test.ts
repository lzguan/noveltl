...
function renderController(role: "owner" | "editor" | "viewer" = "owner"): Harness {
    ...
    const getMode = () => mode;
    const runtime = buildRuntime(setErrors, makeNovel(), editChapterData.chapter, editChapterData, "user-1");
...
function labelByWord(runtime: Runtime, labelGroupId: ProvisionalId, word: string) {
    const label = runtime.dataManager.getForGroup.labels(labelGroupId).find((candidate) => candidate.labelWord === word);
    if (!label) {
