...
export type EditorMode = "edit" | "label" | "view";
/**
...
export type DecoratedSignal = _DecoratedSignal & {
  [DecoratedSignalTag]: typeof DecoratedSignalTag;
};
...
type CachedResultOutput =
  | { signal: Signal; status: "success"; error: null }
  | { signal: null; status: "pending"; error: null }
  | { signal: null; status: "failure"; error: Error };
...
export type IdentifiableKindMap = {
  [K in IdentifiableKind]: Map<
    ProvisionalId,
    { serverId: ServerId | null; status: IdStatus; lockCount: number }
  >;
};
export type ExistableKindMap = {
  [K in ExistableKind]: Map<
    ProvisionalId,
    { serverExists: ServerExists | null; status: IdStatus; lockCount: number }
  >;
};
...
const isRecordOf =
  <T>(valueValidator: Validator<T>): Validator<Record<string, T>> =>
