...
export function buildProvisionalChapterContentId(
...
export function buildDataEntries(
  ...
  provisionalChapterContentId: ProvisionalId,
): DataEntry[] {
  return editChapterData.labelGroupList.map((labelGroupEntry): DataEntry => {
    const labelGroupProvisionalId = idRepo.newIdAndBindId(
      "labelGroup",
      labelGroupEntry.labelGroup.labelGroupId,
    );
    ...
    return {
      labelGroup: {
        ...labelGroupEntry.labelGroup,
        labelGroupId: labelGroupProvisionalId,
        provisional: true,
      },

      labelData: labelGroupEntry.labelData
        ? {
            ...labelGroupEntry.labelData,
            labelDataId: provisionalLabelDataId!,
            provisional: true,
          }
        : (() => {
            ...
            requestManager.enqueueRequest({
              ...
              reservationRequest: {
                reserveList: [
                  ...
                  {
                    kind: "chapterContent",
                    id: provisionalChapterContentId,
                    desiredState: "locked",
                  },
                ],
              },
              ...
              callback: async (requestKey) => {
                ...
                try {
                  response = await createLabelDataLabelGroupsLabelGroupIdLabelDatasPost({
                    body: {
                      chapterContentId: idRepo.getServerId(
                        "chapterContent",
                  ...
                  throw new ConnectionError(
                    "Failed to create label data on server",
                    err instanceof Error ? { cause: err } : undefined,
                  );
                }
                if (response.error) {
                  if (
                    isRequestConflictErrorResponse(response.error) &&
                    response.error.detail.cacheConflict
                  ) {
                  ...
                  throw new FatalError(
                    `Server returned an error while creating label data: ${response.error instanceof Error ? response.error.message : JSON.stringify(response.error)}`,
                  );
                } else {
            ...
            return {
              labelGroupId: labelGroupProvisionalId,
              labelDataId: provisionalId,
              chapterContentId: provisionalChapterContentId,
              provisional: true,
            };
          })(),

      labels: labelGroupEntry.labelData
        ? editChapterData.labelDataList
            .find((labelData) => labelData.labelDataId === labelGroupEntry.labelData!.labelDataId)
            ?.labels.map((label) => ({
              ...label,
      ...
      loadingStatus: editChapterData.labelDataList.some(
        (val) => val.labelDataId === labelGroupEntry.labelData?.labelDataId,
      )
        ? "loaded"
        : "notLoaded",
    };
...
export function buildRuntime(
  ...
  const entries = buildDataEntries(
    editChapterData,
    idRepo,
    requestManager,
    provisionalChapterContentId,
  ...
  const colorMapping = new Map<ProvisionalId, Color>(
    entries.map((entry) => [entry.labelGroup.labelGroupId, generateRandomColor()]),
  );
  const visibilityMapping = new Map<ProvisionalId, boolean>(
    entries.map((entry) => [entry.labelGroup.labelGroupId, false] as [ProvisionalId, boolean]),
  );
...
export function buildLabelGroupViews(
  ...
  return dataManager.getGroups().map(
    (group): LabelGroupView => ({
