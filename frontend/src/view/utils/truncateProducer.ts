function truncateProducer(
	maxLength: number,
): (t: string) => { truncated: string; canTruncate: boolean } {
	function truncateParagraph(t: string): { truncated: string; canTruncate: boolean } {
		...
		if (paragraphs.length === 1) {
			const truncated =
				paragraphs[0].length > maxLength
					? paragraphs[0].slice(0, maxLength).concat("...")
					: paragraphs[0];
