...
def test_label_insert_label_data_by_autolabels_basic(
    ...
    res = insert_label_datas_by_autolabels(db_session, sample_users[0], chinese_xianxia_small_test_label_group.label_group_id, request)
    ...
    label_datas_in_db = db_session.execute(
...
## ---------------- Populate test data ---------------- ##
# @pytest.mark.asyncio
# async def test_chinese_xianxia_small_test_autolabels(chinese_xianxia_small_test_novel : Novel, chinese_xianxia_small_test_chapters : List[Tuple[RawChapter, RawChapterRevision]], autolabel_loader : Loader, db_session : Session, worker_mock : Worker, redis, sample_users : List[User]):
