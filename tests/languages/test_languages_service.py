...
def language_session_test(test_db : Session) -> Dict[str, Language]:
...
def test_query_language_by_id(test_db : Session, language_session_test : Dict[str, Language]):
    lang_en = language_session_test['en']
    queried_lang = query_language_by_id(test_db, language_session_test['en'].language_id)
    ...
    queried_lang = query_language_by_id(test_db, language_session_test['fr'].language_id)
    ...
    with pytest.raises(LanguageNotFoundException):
        query_language_by_id(test_db, largest_id + 1)  # Non-existent ID

def test_query_language_by_code(test_db : Session, language_session_test : Dict[str, Language]):
...
def test_query_all_languages(test_db : Session, language_session_test : Dict[str, Language]):
